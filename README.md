...
> 🦝 **核心賣點：全自動。** 你在 Threads「按收藏」的貼文 → bot 每 6 小時自動爬下來 → Claude AI 整理成 Notion 卡片（摘要 / 分類 / 標籤 / 圖片）。**你只要按收藏，剩下都自動。**
>
> ⚠️ 但自動整理只解了一半 —— 還是要自己打開 Notion 來看、再整理、把學到的東西實踐出來，不然 Notion 一樣是個黑洞。
...
flowchart LR
    ...
    THREADS_FAV --> SYNC
    SYNC --> CLAUDE1[Claude Haiku<br/>摘要/分類/標籤]
...
傳任何網址或文字給 Telegram bot，自動爬內容（Playwright）→ Claude 分析（摘要／分類／標籤）→ 寫入 Notion Database。
